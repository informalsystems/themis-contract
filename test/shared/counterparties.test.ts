...
const TEST_COUNTERPARTY_CORRECT = `full_name = "Counterparty A"
signatories = [
  "sig01",
  "sig02"
]
...
describe('Counterparty', () => {
  describe('#loadFromFile()', () => {
    it('should parse a well-formed counterparty', async () => {
      const tmpDir = tmp.dirSync()
      const tmpFilename = path.join(tmpDir.name, 'cp01.toml')
      await writeFileAsync(tmpFilename, TEST_COUNTERPARTY_CORRECT)
